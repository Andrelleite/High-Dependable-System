        ...
        if(usersToContact.size() != 0){
            ...
            while (i.hasNext()) {
                ...
                try {
                    ...
                    // 1,N BEGIN
                    this.writerTimeStamp.addAndGet(1);
            ...
            public void run(){
                ...
                try {
                    for(ServerInterface s: netToSym.keySet()){
                        ...
                        ServerReturn r = s.obtainLocationReport(tempC,loc3,tempC.getUsername(),rid[0]);
            ...
            public void run(){
                ...
                try {
                    for(ServerInterface s: netToSym.keySet()){
                        ...
                        ServerReturn r = s.requestMyProofs(tempC,tempC.getUsername(),loc3,loc4,rid[0]);
    ...
    public int verifiySignatureW(ArrayList<Report> reportList, ServerInterface sr, int flag){
        for(int j = 0; j < reportList.size(); j++){
            ...
            try {
                ...
                String s1 = reportList.get(j).getUsername() + reportList.get(j).getNonce() + reportList.get(j).getTimeStamp() + reportList.get(j).getEpoch() + reportList.get(j).getPosX() + reportList.get(j).getPosY();
